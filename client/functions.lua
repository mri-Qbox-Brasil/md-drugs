...
function ItemCheck(item)
    ...
    if QBCore.Functions.HasItem(item) then
        success = item
        return success
    else Notify('You Need ' .. QBCore.Shared.Items[item].label .. " !", 'error') end
end
...
function GetCops(number)
    ...
    QBCore.Functions.TriggerCallback('md-drugs:server:GetCoppers', function(amount)
        if amount >= number then need = true else
            Notify('You Need ' .. number - amount .. ' More Cops To Do This', 'error')
            nope = true
        end
    end)
    repeat
