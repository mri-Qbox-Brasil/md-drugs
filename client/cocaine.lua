...
CreateThread(function()
    ...
    if Config.oxtarget then
        exports.ox_target:addBoxZone({ coords = vector3(1093.17, -3195.74, -39.19 - 1), size = vec3(1, 1, 1), options =
        options })
        exports.ox_target:addBoxZone({ coords = vector3(1093.17, -3195.74, -39.19 - 1), size = vec3(1, 1, 1), options =
        options2 })
    else
        exports['qb-target']:AddBoxZone("cutcokepowder", vector3(1093.17, -3195.74, -39.19 - 1), 1.5, 1.75,
            { name = "cutcokepowder", minZ = -40.0, maxZ = -38.0, }, { options = options, distance = 2.0 })
        exports['qb-target']:AddBoxZone("bagcokepowder", vector3(1090.29, -3195.66, -39.13 - 1), 1.5, 1.75,
            { name = "bagcokepowder", minZ = -40, maxZ = -38, }, { options = options2, distance = 2.0 })
    end
    if Config.FancyCokeAnims == false then
        for k, v in pairs(Config.CuttingCoke) do
            ...
            if Config.oxtarget then
                ...
                exports['qb-target']:AddBoxZone("cutcoke" .. k, vector3(v.loc.x, v.loc.y, v.loc.z), v.l, v.w,
                    { name = "cutcoke" .. k, heading = 156.0, minZ = v.loc.z - 1, maxZ = v.loc.z + 1, },
        ...
        for k, v in pairs(Config.BaggingCoke) do
            ...
            if Config.oxtarget then
                ...
                exports['qb-target']:AddBoxZone("bagcoke" .. k, vector3(v.loc.x, v.loc.y, v.loc.z), v.l, v.w,
                    { name = "bagcoke" .. k, heading = 156.0, minZ = v.loc.z - 1, maxZ = v.loc.z + 1, },
                    { options = options, distance = 1.5 })
            end
        end
    end
